...
| 4–5 | **Problem catalog + frontend** | Problem/testcase tables, custom LRU cache (hashmap + doubly linked list) in front of hot reads, token-bucket rate limiter with idempotency keys and 429/Retry-After backpressure, React frontend with Monaco editor. **Completed**: Problem/testcase tables verified, custom LRU cache implemented, token-bucket rate limiter with idempotency keys and 429/Retry-After backpressure implemented. **Remaining**: React frontend with Monaco editor integration. |
